...
def show_explanation(result: dict):
    ...
    st.write("This result is generated by the Python implementation of the updated fit10 Cox model.")
    st.write("The displayed risk group follows the manuscript-defined cutoff derived from the nomogram total points rather than an arbitrary probability threshold.")
...
with st.sidebar:
    st.write("Prediction engine: Python fit10")
    if deepseek_ready():
...
st.markdown("---")
st.caption("Note: AI is used for structured extraction and patient education only. Risk values are calculated by the Python implementation of the fit10 model. Risk grouping follows the manuscript-defined nomogram cutoff.")
